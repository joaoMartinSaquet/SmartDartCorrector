...
def sample_sac_params(trial: optuna.Trial) -> Dict[str, Any]:
    ...
    return {
        ...
        "tau": trial.suggest_float("tau", 0.005, 0.015),
        "gamma": trial.suggest_float("gamma", 0.90, 0.9999),
        "gradient_steps": trial.suggest_int("gradient_steps", 10, 100),
        "ent_coef": trial.suggest_categorical("ent_coef", ["auto", 0.0, 0.01]),
    }
...
def train_once(args: argparse.Namespace,nstack = 1, **model_kwargs) -> tuple[SAC, float]:
    ...
    vec_env = make_vec_envs(perturbator, args.n_envs, args.render, args.normalize, nstack=nstack)
    policy_kwargs = dict(
    ...
    model = SAC(
        ...
        **model_kwargs,
        ...
        # learning_starts=args.lealearning_startsrning_starts,
    )
...
def main():
    ...
    # Custom network architecture Tanh before 
    policy_kwargs = dict(
    net_arch=[256, 256],  # Two hidden layers of 256 units each
    activation_fn=th.nn.Tanh,  # Use ReLU activation function
    ) 
    ...
    if args.optuna_trials > 0:
        ...
        path = f'logs_corrector/' + args.perturbator + f'SAC_{time.strftime("%Y%m%d-%H%M%S")}/'
        ...
        ckpt_cb = CheckpointCallback(save_freq=10000, save_path="models/sac", name_prefix="sac_sd")
