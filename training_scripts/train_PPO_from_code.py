...
if TUNE_HYPERPARAMETERS:
    ...
    def obejectivePPO(trial):
        ...
        decay =  trial.suggest_float("action_std", 0.05, 0.1)
        min_action_std = trial.suggest_float("min_action_std", 0.0  , .1)
        update_factor = trial.suggest_int("update_factor", 1, 10)
        perturbator = None
        u_sim = VITE_USim([0, 0])
        env = smartDartEnv(None, None, render = False, n_parallel=1)
        env.set_usim(u_sim)
        env.set_perturbator(perturbator)
        
        logger.info(f"Training on {env}")
        corr = PPOCorrector(env, u_sim, perturbator,
                             ...
                             decay_action_std=decay, min_action_std=min_action_std, update_factor=update_factor, max_training_timesteps=2.5e6, max_ep_len=1000)
        log = corr.train()
        df = pd.DataFrame(log)
...
if __name__ == "__main__":
    ...
    if TUNE_HYPERPARAMETERS:
        ...
        study.optimize(obejectivePPO, n_trials=20, n_jobs=1, show_progress_bar=True)
        ...
        for i in range(N):
            ...
            corrector = PPOCorrector(env, u_sim, perturbator, hidden_size=64, 
                                     lr_actor=3e-4, lr_critic=1e-3, gamma=0.99, k_epochs=80, clip_epsilon=0.2, gae_lambda=1, 
                                     action_std_init = 0.3, decay_action_std = 0.05, min_action_std = 0.01, max_training_timesteps = 2.5e6, 
                                     max_ep_len = 1000, update_factor = 4, policy_type="StackedMLP", seq_length=10)
            log = corrector.train()
